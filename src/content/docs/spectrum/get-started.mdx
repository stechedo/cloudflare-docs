...
4. **Make sure the origin is reachable.** The origin must accept traffic from Cloudflare. If the origin is private, you have two options for non-HTTP traffic: attach Spectrum directly to a [virtual network origin](/spectrum/get-started/#create-a-spectrum-application-using-a-virtual-network-origin) (simplest), or route through a [Cloudflare Load Balancer with Private Network Load Balancing](/load-balancing/private-network/) when you also need load balancing. Using Cloudflare Tunnel directly as a Spectrum origin is only supported for [HTTP/HTTPS applications](/spectrum/reference/limitations/#cloudflare-tunnel).
