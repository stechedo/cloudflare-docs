...
4. **Make sure the origin is reachable.** The origin must accept traffic from Cloudflare. If the origin is private and you want to use Cloudflare Tunnel, remember that Tunnel is only supported directly for HTTP/HTTPS Spectrum applications. For non-HTTP traffic, route Spectrum through a [Cloudflare Load Balancer with Private Network Load Balancing](/load-balancing/private-network/) instead.
